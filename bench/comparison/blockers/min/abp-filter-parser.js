...
function matchOptions(filterOptions, input, contextParams, currentHost) {
  ...
  if (
    filterOptions.elementType !== contextParams.elementType &&
    filterOptions.elementType !== undefined
  ) {
    return false;
  }
  if (
    filterOptions.skipElementType === contextParams.elementType &&
    filterOptions.skipElementType !== undefined
  ) {
    return false;
  }
...
function matchesFilters(filters, input, contextParams) {
  ...
  if (nonAnchoredStringMatches.length !== 0) {
    ...
    for (i = 0; i < len; i++) {
      if (
        indexOfFilter(input, nonAnchoredStringMatches[i].data, 0) !== -1 &&
        matchOptions(nonAnchoredStringMatches[i].options, input, contextParams, currentHost)
      ) {
...
function matches(filters, input, contextParams) {
  ...
  if (
    matchesFilters(filters, input, contextParams) &&
    !matchesFilters(filters.exceptionFilters, input, contextParams)
  ) {
    return true;
  }
  return false;
