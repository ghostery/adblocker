  ...
  describe('#getCosmeticsFilters', () => {
    describe('script injections', () => {
      it('injects script', () => {
        ...
        ).to.eql([wrapScriptletBody('function script() {}\nscript').replace('{{1}}', 'arg1')]);
      });
...
`,
      ...
      it('disabling specific hides does not impact scriptlets', () => {
        ...
        engine.resources.scriptletCaches.set('foo', '');
        expect(
          engine.getCosmeticsFilters({
            domain: 'foo.com',
            hostname: 'foo.com',
...
##selector1 :style(foo)`,
    ...
    [
      ...
      ({
        ...
        it(JSON.stringify({ filters, hostname, matches, injections }), () => {
          ...
          const engine = createEngine(filters.join('\n'));
          engine.resources.scriptletCaches.set('scriptlet', 'scriptlet');
          engine.resources.scriptletCaches.set('scriptlet1', 'scriptlet1');
          engine.resources.scriptletCaches.set('scriptlet2', 'scriptlet2');
  ...
  describe('#merge', () => {
    ...
    context('with resources', () => {
      ...
      it('merge engines with first resource', () => {
        const engine1 = FilterEngine.empty();
        const engine2 = FilterEngine.empty();
        ...
        const engine = FilterEngine.merge([engine1, engine2]);
        expect(engine.resources.scriptletCaches.size).to.be.eql(2);
